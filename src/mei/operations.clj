...
(def present-value
  (+ principal interest-at-due-date))
...
(defn ->charge [index due-date amount status]
  {:index index
   :due-date due-date
   :amount amount
   :status status})
...
; each array element.
(def charges
  [(->charge 1 "2023-10-03" 130M :paid)
   (->charge 2 "2023-11-03" 1000M :due)
   (->charge 3 "2023-12-03" 2000M :due)
   (->charge 4 "2024-01-03" 100M :due)
   (->charge 5 "2024-02-03" 300M :due)])

(println charges)

; Now that we have the charges, what about associate a new key on our `loan` with the charges?
(def loan-with-charges (assoc loan :charges charges))
(println loan-with-charges)
